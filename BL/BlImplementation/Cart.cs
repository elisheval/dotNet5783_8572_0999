...
internal class Cart : ICart
{
    ...
    {
        DO.Product product1 = _dal.Product.GetByCondition((x) => x!=null&&productId == x.Value.Id);
    ...
    public BO.Cart AddProductToCart(int productId, BO.Cart myCart)
    {
        ...
        foreach (var orderItem in myCart.OrderItemList)
        {
            if (orderItem.ProductId == productId)
            {
                ...
                {
                    DO.Product product = _dal.Product.GetByCondition(x=>x!=null&&x.Value.Id==productId);
        ...
        DO.Product product1 = _dal.Product.GetByCondition(x => x!=null&& x.Value.Id == productId);
        if (product1.InStock <= 0)
            throw new BO.ProductOutOfStockException("Product is out of stock");
                        ...
                        if (product != null)
                        {
                            if (product.Value.Id == productId)
                            {
                                if (product.Value.InStock == 0)
                                    throw new BO.ProductOutOfStockException("product is out of stock");
                                else if (product.Value.InStock >= newAmount)
                                {
                                    myCart.TotalOrderPrice += product.Value.Price * newAmount - orderItem.TotalPriceForItem;
                                    orderItem.TotalPriceForItem = newAmount * product.Value.Price;
                                ...
                                {
                                    if (product.Value.InStock > orderItem.AmountInCart)
                                    {
                                        myCart.TotalOrderPrice += product.Value.Price * product.Value.InStock - orderItem.TotalPriceForItem;
                                        orderItem.TotalPriceForItem = product.Value.InStock * product.Value.Price;
                                        orderItem.AmountInCart = product.Value.InStock;
                                        orderItem.Price = product.Value.Price;
                                        return myCart;
                                    }
...
public void OrderConfirmation(BO.Cart myCart, string customerName, string customerEmail, string customerAddress)
{
    ...
    foreach (BO.OrderItem orderItem in myCart.OrderItemList)
    {
        try
        {
            DO.Product productFromDo = _dal.Product.GetByCondition(x=>x!=null&&x.Value.Id==orderItem.ProductId);
